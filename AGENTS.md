...
- **SSID cache allocation pattern**: The 64-entry BSSID→SSID cache (2.75KB) must also be in internal RAM (same PSRAM bus stall reason as sprites). But static `.dram0.bss` placement reduces free heap at boot, pushing it below the 91KB sprite allocation threshold. `.ext_ram.bss` is silently ignored by the linker (falls back to `.dram0.bss`). `.psram_bss` puts it in PSRAM → re-creates the bus stall TG1WDT. **Solution**: dynamically allocate from internal heap via `heap_caps_calloc(SSID_CACHE_SIZE, sizeof(SsidCacheEntry), MALLOC_CAP_INTERNAL)` in `NetworkRecon::start()`, free in `stop()`. Since `start()` runs after `Display::init()` (which allocates the sprite), the 2.75KB comes from remaining heap, not the sprite's budget. See `src/core/network_recon.cpp`.
