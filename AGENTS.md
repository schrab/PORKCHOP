...
- **Dynamic internal RAM allocation pattern (SSID cache & pendingUpdates)**: Large recon structures (the 2.75KB 64-entry BSSID→SSID cache and the 1.66KB 32-slot `pendingUpdates` queue) must be in internal RAM (same PSRAM bus stall reason as sprites). But static `.dram0.bss` placement reduces free contiguous internal heap at boot, pushing it below the 90.88KB sprite allocation threshold required by `mainCanvas.createSprite(320, 142, 16)`. `.ext_ram.bss` is silently ignored by the linker (falls back to `.dram0.bss`). `.psram_bss` puts it in PSRAM → re-creates the bus stall TG1WDT. **Solution**: dynamically allocate from internal heap via `heap_caps_calloc(..., MALLOC_CAP_INTERNAL)` in `NetworkRecon::start()`, free in `stop()`. Since `start()` runs after `Display::init()` (which allocates the sprite), these allocations come from remaining heap, not the sprite's budget. See `src/core/network_recon.cpp`.
- **WiFi dynamic TX buffer pool (framework patch)**: Arduino's `WiFiGeneric.cpp` hardcodes `dynamic_tx_buf_num=32` at `esp_wifi_init()`, ignoring sdkconfig. OINK's deauth burst rate exceeds 32 descriptors in flight, causing `esp_wifi_80211_tx()` to return `ESP_ERR_NO_MEM` (257) on most calls. Patched via `scripts/patch_wifi_buffers.py` to set `dynamic_tx_buf_num=128`. Auto-applied on every build by `scripts/pre_build.py` at SCons env setup (idempotent). **Lost on `pio run --target clean`** — re-apply via `pio run` (no clean) or document in `docs/err257_wdt_fix_log.md`. RX/cache/static NOT bumped — that adds ~12.8KB internal RAM which broke the 90KB main canvas sprite allocation. See `docs/err257_wdt_fix_log.md` for full analysis.
- **Interrupt Watchdog framework patch**: Arduino's pre-built `libesp_system.a` bakes `CONFIG_ESP_INT_WDT_TIMEOUT_MS=300` (300ms). `board_build.sdkconfig` is IGNORED for pre-built framework libs. `scripts/pre_build.py::patch_int_wdt()` patches the framework `sdkconfig.h` to 5000ms. Auto-applied on every build (idempotent). Survives clean builds. Lost on `pio pkg update` or framework upgrade.
- **WiFi.disconnect() always needs explicit params**: `WiFi.disconnect()` defaults to `eraseap=true` which calls `esp_wifi_set_config()` internally — this can reset the dynamic TX buffer pool and corrupt WiFi driver state. Always use `WiFi.disconnect(false, false)` unless you intentionally want to erase saved AP credentials. Applied in both `NetworkRecon::start()` and `NetworkRecon::resume()`.
- **BORED state lock throttle**: `getNextTarget()` in OINK's BORED state iterates all ~60 networks under `vectorMux`. Was called every loop iteration (~50Hz), causing 210 lockCalls/2s. Throttled to every 2s via `lastBoredTargetCheck` timestamp. Core 0 heartbeat diagnostic (`c0pkt=N` in OINK-DIAG) confirms Core 0 stays alive during BORED. See `docs/tg1wdt_investigation.md` for full analysis.
