...
void CrashViewer::drawList(DisplayCanvas& canvas) {
...
void CrashViewer::drawFile(DisplayCanvas& canvas) {
    canvas.fillSprite(COLOR_BG);
    canvas.setTextColor(COLOR_FG, COLOR_BG);
    canvas.setTextSize(1);
    canvas.setFont(NULL);
...
void CrashViewer::drawNukeConfirm(DisplayCanvas& canvas) {
    const int boxW = 200;
    const int boxH = 70;
...
void CrashViewer::draw(DisplayCanvas& canvas) {
    if (!active) return;
